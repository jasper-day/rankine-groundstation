...
def test_get_params():
    ...
    assert cs1.get_params() == approx(
        np.array([15.0, 20.0, 5.0, np.pi, -5.0 * np.pi / 2.0])
    )
    assert ls2.get_params() == approx(np.array([15.0, 25.0, 35.0, 25.0]))
    assert cs2.get_params() == approx(
        np.array([35.0, 35.0, 10.0, -np.pi / 2.0, 10.0 * np.pi / 2.0])
    )
    assert cs3.get_params() == approx(np.array([50.0, 35.0, 5.0, np.pi, -5.0 * np.pi]))


def test_get_params_with():
    for segment in [ls1, ls2]:
        assert segment.start() == approx(segment.start_with(segment.get_params()))
        assert segment.end() == approx(segment.end_with(segment.get_params()))
        assert segment.heading_start() == approx(
            segment.heading_start_with(segment.get_params())
        )
        assert segment.heading_end() == approx(
            segment.heading_end_with(segment.get_params()), abs=1e-6
        )
    for segment in [cs1, cs2, cs3]:
        assert segment.start() == approx(segment.start_with(segment.get_params()))
        assert segment.end() == approx(segment.end_with(segment.get_params()))
        assert segment.heading_start() == approx(
            segment.heading_start_with(segment.get_params())
        ...
        assert segment.heading_end() == approx(
            segment.heading_end_with(segment.get_params())
        )
...
def test_headings():
    assert ls1.heading_end() == approx(ls1.heading_start(), abs=1e-6)
    assert ls1.heading_end() == approx(np.pi / 2, abs=1e-6)
    assert cs1.heading_start() == approx(np.pi / 2, abs=1e-6)
    assert cs1.heading_end() == approx(0.0, abs=1e-6)
    assert ls2.heading_end() == approx(ls2.heading_start(), abs=1e-6)
    assert ls2.heading_end() == approx(0.0, abs=1e-6)
    assert cs2.heading_start() == approx(0.0, abs=1e-6)
    assert cs2.heading_end() == approx(np.pi / 2, abs=1e-6)
    assert cs3.heading_start() == approx(np.pi / 2, abs=1e-6)
    assert cs3.heading_end() == approx(-np.pi / 2, abs=1e-6)


def test_evals():
    # inside segment
    assert ls1.eval(5) == approx(np.array([10, 5]))
    assert cs1.eval(5 * np.pi / 4) == approx(
        np.array([15, 20]) + 5 * np.array([-sqrthalf, sqrthalf])
    )
    assert ls2.eval(10) == approx(np.array([25, 25]))
    assert cs2.eval(10 * np.pi / 4) == approx(
        np.array([35, 35]) + 10 * np.array([sqrthalf, -sqrthalf])
    )
    assert cs3.eval(5 * np.pi / 4) == approx(
        np.array([50, 35]) + 5 * np.array([-sqrthalf, sqrthalf])
    )
    assert cs3.eval(5 * 3 * np.pi / 4) == approx(
        np.array([50, 35]) + 5 * np.array([sqrthalf, sqrthalf])
    )
    # outside segment
...
def test_path_length():
    assert path.length() == approx(
        ls1.length() + cs1.length() + ls2.length() + cs2.length() + cs3.length()
    )
